...
<?php
	...
	if($conn->connect_error)
		...
		$sql="select * from classincharge where tid=$tid;";
	 ...
	 if($rows==0)
	 {
		...
		$sqljoin.='where student.sem='.$sem.' and student.sec="'.$sec.'";';
	 ...
	 if($joinrows==0)
	 {
		...
		echo "<div id='tab'><table style='overflow: scroll;overflow:auto;'><tr><th></th>";
		for($i=0;$i<$rows;$i++)
		{
		...
		echo "</tr><tr><th class='fixedcol'>Regno.</th>";
		for($i=0;$i<$rows;$i++)
		{
		...
		for($i=0;$i<$joinrows;$i++)
		{
			...
			echo "<tr class='rows'><td class='fixedcol'>".$joinrow['Regno']."</td>";
			for($j=1;$j<=$rows;$j++)
			{
